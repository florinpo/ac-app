    ...
    public function actionMarkspam() {
        if (Yii::app()->request->isPostRequest) {
            ...
            foreach ($_POST['convs'] as $conversation_id) {
                ...
                if ($conv->partialConversation(user()->id, $folder)) {
                    if ($conv->markSpamMessages(user()->id, $folder))
                        $count++;
                } else {

                    if (!$conv->belongsTo(user()->id))
                        continue;
                    if ($conv->markSpam(user()->id) || $conv->validate())
                        $count++;
                }
            }
            if ($count) {
    ...
    public function actionMarknospam() {
        if (Yii::app()->request->isPostRequest) {
            ...
            foreach ($_POST['convs'] as $conversation_id) {
                ...
                if (!$conv->belongsTo(user()->id))
                    continue;
                if (!$conv->restoreMessages(user()->id, $folder))
                    continue;
                if (!$conv->validate())
                    continue;
                if ($conv->unmarkSpam(user()->id))
                    $count++;
            }
            if ($count) {
                if ($count > 1) {
...
//                    continue;
//
//                if ($conv->permanentDelete(user()->id))
//                    $count++;

                if ($conv->partialConversation(user()->id, $folder)) {
                    if ($conv->permanentDeleteMessages(user()->id, $folder))
                        $count++;
                } else {
