(function($) {
        ...
        $.ajax({
            ...
            success: function(response){
                if(response.success) {
                    ...
                    var max = Math.max.apply(Math, response.result);
                    var ticks = [];
                    
                    var q = (Math.floor(response.result.length/31));
                    
                    //console.log(l);
                    
                    if(l > 31){
                       
                        for (var i = 0; i < l-q; i += q) {
                            points.push(response.result[i]);
                        }
                        
                        
                        if(l>31 && l<=89){
                            if($.inArray( response.result[l-q], points )==-1)
                                points.push(response.result[l-1]);
                        } else if(l>89) {
                            if($.inArray( response.result[l-q], points )==-1)
                                points.push(response.result[l-q]);
                        }
    ...
    $.statistics.drawMonthLabels = function(plot, points) {
        ...
        $.each(pointsGroup, function(key, v){
            ...
            var minLength = (points.length / 31);
            posL = offset.left;
            posT = plot.height()+35;
    ...
    $.statistics.drawGraph = function(points) {
        ...
        $.statistics.drawMonthLabels(plot, points);
