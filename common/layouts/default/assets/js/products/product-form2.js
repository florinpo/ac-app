...
(function($) {
    ...
    $.productForm.updateProduct = function() {
        ...
        $('#ProductSaleForm_domain_id').chosen({
         ...
         $('#ProductSaleForm_category_id').trigger('chosen:activate');
...
//        
    ...
    $.productForm.updateSelected = function(){
        
        var selectedList = $("#selected-items");
        
        
        var current_domain = $("ul#p-domains input:checked");
        var domain_label = $.trim(current_domain.closest('li').text());
           
        var current_category = $("ul#p-categories input:checked");
        var category_label = $.trim(current_category.closest('li').text());
        var category_val = current_category.val();
           
        var current_subcategory = $("ul#p-subcategories input:checked");
        var subcategory_label = $.trim(current_subcategory.closest('li').text());
        var subcategory_val = current_subcategory.val();
        
        var label;
        var current_id;
        
        if(current_subcategory.length == 0){
            label = domain_label + " > " + category_label;
            current_id = category_val;
        } else {
            label = domain_label + " > " + category_label + " > " + subcategory_label;
            current_id = subcategory_val;
        }
        
        if($.inArray(current_id, $.productForm.selectedIds)==-1){
            $.productForm.selectedIds.push(current_id);
            var output = "<li id='s-" + current_id + "' class='t-cancel'><span class='icon icon-remove-sign'></span>" + label + "</li>";
            selectedList.append(output);
            $("#ProductSaleCategoryForm_selected_categories").val($.productForm.selectedIds);
            $('.t-cancel').tooltipster({
                trigger: 'hover',
                position: 'right',
        ...
        if(selectedList.find('li').length>0){
            $('.s-wrapper .empty').hide();
        }
        
        if(selectedList.find('li').length == 3){
            selectedList.find('li:last-child').addClass('last');
        }
        
    }
    
    $.productForm.getScrollbars = function(selector, length){
        
        if(length == 5) {
            selector.find('li:last-child').addClass('last');
        }
        // we activate the scroll only if we have more than 5 items
        if(length > 5){
            selector.slimScroll({
                height: '175px',
                wheelStep: 10,
